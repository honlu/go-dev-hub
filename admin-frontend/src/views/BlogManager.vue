  ...
  <div>
    <el-card>
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
        <div>
          <el-input v-model="search.title" placeholder="搜索标题" style="width: 200px; margin-right: 8px;" @keyup.enter="handleTitleSearch" />
        ...
        <el-button type="primary" @click="openEdit()">新建博客</el-button>
      </div>
    ...
    <!-- 详情弹窗 -->
    <el-dialog title="博客详情" v-model="detailVisible" width="900px">
      <div v-if="detailData">
    ...
    <!-- 编辑弹窗 -->
    <el-dialog :title="editForm.id ? '编辑博客' : '新建博客'" v-model="editVisible" width="800px">
      <el-form :model="editForm" label-width="80px">
        <el-form-item label="标题" required>
          <el-input v-model="editForm.title" placeholder="请输入博客标题" />
        </el-form-item>
        ...
        <el-form-item label="内容" required>
          <el-input 
            ...
            :rows="12" 
            placeholder="支持 Markdown 格式，请输入博客内容" 
          />
        </el-form-item>
      </el-form>
      <template #footer>
...
const search = reactive({ title: '', category: '', status: '' });
const categoryOptions = ref([]);
const categoryTags = ref({});

const editVisible = ref(false);
const editForm = reactive({ id: null, title: '', category: '', tags: [], status: 'draft', content_md: '' });
...
const renderedContent = computed(() => {
  if (!detailData.value || !detailData.value.content_md) {
    return '';
  }
  return marked(detailData.value.content_md);
});
...
function openEdit(row) {
  if (row) {
    ...
    Object.assign(editForm, { id: null, title: '', category: '', tags: [], status: 'draft', content_md: '' });
  }
...
function handleDelete(id) {
  ElMessageBox.confirm('确定要删除该博客吗？', '提示', { type: 'warning' })
    .then(() => {
      deleteBlog(id).then(() => {
